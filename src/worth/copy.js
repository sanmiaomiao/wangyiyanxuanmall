{/*   {
                    this.state.topics.length>=1 &&this.state.topics.map((item,index)=>{
                        var WH=1;
                        Image.getSize(item.picUrl,(width,height)=>{
                            console.log(width)
                            console.log(height)
                            WH=height/width;
                            console.log(WH)
                        ...
                        return(
                                <View style={{marginLeft:5,marginTop:10,borderRadius:20,borderWidth:1,borderColor:'transparent', overflow:'hidden',minHeight:200,backgroundColor:'white',width:width/2.2}}>
                                   <Image source={{uri:item.picUrl}} style={{width:'100%',height:(WH-0.4)*(width/2.2), backgroundColor:'red'}}/>
        ...
        return (
            <View
                onStartShouldSetResponderCapture={() => {
                    ...
                    /*    if (this.state.scrollOffset <=200
                            && this.state.enableScrollViewScroll === false) {
                            this.setState({ enableScrollViewScroll: true });
                            this.myFlat.current.scrollToEnd();
                        }*/
                }}>
                <FlatList
